...
class PaymentDelayView extends StatelessWidget {
  const PaymentDelayView({super.key, this.orderId});

  /// The orderId is available if the order was created before the timeout.
  /// Null if something went wrong before the order was persisted.
  final String? orderId;

  @override
  Widget build(BuildContext context) {
    return PopScope(
      // Nothing to pop back to — always intercept and go Home.
      canPop: false,
      onPopInvokedWithResult: (_, _) => context.go(RoutePaths.home),
      child: Scaffold(
                ...
                CustomButton(
                  onPressed: () {
                    if (orderId != null) {
                      ...
                      // Fallback: orderId unavailable, go to the orders list
                      context.go(RoutePaths.orders);
                    }
                  },
                  borderRadius: 16.r(context),
                  child: Text(
                    orderId != null ? 'View Order' : 'View My Orders',
